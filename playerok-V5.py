...
# Flask сервер для Railway - скрытый обход
app = Flask(__name__)

# Отключаем логи Flask для полной невидимости
import logging
log = logging.getLogger('werkzeug')
log.setLevel(logging.ERROR)
app.logger.disabled = True

# Скрываем Flask заголовки
@app.after_request
...
@app.route("/")
def home():
    return "", 200  # Пустой ответ

@app.route("/ping")
def ping():
    return "", 200  # Для uptime мониторинга
...
@app.route("/<path:path>")
def catch_all(path):
    return "", 404  # Все неизвестные пути возвращают 404

# Функция для запуска Flask в отдельном потоке
def run_flask():
    try:
        port = int(os.environ.get("PORT", 8080))
        # Запускаем без вывода логов и в продакшн режиме
        import werkzeug
        werkzeug.serving.WSGIRequestHandler.log_request = lambda self, *args, **kwargs: None
        app.run(host="0.0.0.0", port=port, debug=False, use_reloader=False, threaded=True)
    except Exception as e:
        print(f"Flask error (игнорируется): {e}")
